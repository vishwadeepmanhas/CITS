...
public class AutomationObject {
    ...
    public static ThreadLocal<HashMap<String,Map<String, Map<String, Map<String, String>>>>> df = new ThreadLocal() {
    	protected HashMap<String, Map<String, Map<String, String>>> initialValue() {
    		return new HashMap<>();
    	}
    };

    public static HashMap<String,Map<String, Map<String, Map<String, String>>>> dynamicValue = df.get();
                ...
                for (ORAttribute attr : attributes) {
                    if (!attr.getValue().trim().isEmpty()) {
                        if (prop == null || attr.getName().equals(prop)) {
                            ...
                            System.out.println("Value of the tag is " + value);
    ...
    private String getRuntimeValue(String value) {
        ...
        if(dynamicValue.containsKey(userData.getTestCase())){
        	       
        	if (dynamicValue.get(userData.getTestCase()).containsKey(pageName)
                && dynamicValue.get(userData.getTestCase()).get(pageName).containsKey(objectName)) {
        		for (String Key : dynamicValue.get(userData.getTestCase()).get(pageName).get(objectName).keySet()) {
        			value = value.replace(Key,
        					dynamicValue.get(userData.getTestCase()).get(pageName).get(objectName).get(Key));
            	}
        	}
        }
    ...
    public void setUserDataAccess(UserDataAccess userData) {
